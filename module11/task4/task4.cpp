...
int compareFunc(std::string num1, std::string num2, int parI, int parJ) {
  ...
  bool point = false;
  for (i = parI;
       i < num1.length() && num1[i] == '0' && num1.length() != 1 + parI; i++) {
  }
  for (j = parJ;
       j < num2.length() && num2[j] == '0' && num2.length() != 1 + parJ; j++) {
  }
  bool endStr1 = false;
  bool endStr2 = false;
  bool zeroCheck = true;
  ...
  for (int k = 0; k < length; k++) {
    if (num1[i] >= '1' && num1[i] <= '9') zeroCheck = false;
  ...
  if (zeroCheck && parI != parJ) return 3;
  return 0;
}
...
int compareNum(std::string num1, std::string num2) {
  if (correctNum(num1) && correctNum(num2)) {
    if (num1[0] != '-' && num2[0] == '-') {
      if (compareFunc(num1, num2, 0, 1) == 3) {
        return 0;
      } else {
        return 1;
      }
    } else if (num1[0] == '-' && num2[0] != '-') {
      if (compareFunc(num1, num2, 1, 0) == 3) {
        return 0;
      } else {
        return -1;
      }
